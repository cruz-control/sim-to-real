...
DrivingServo = kit.servo[15]
SteeringServo = kit.servo[0]

# set initial servo values
DrivingServo.angle = 90
SteeringServo.angle = 90
...
while True:
    ...
    for event in pygame.event.get():
        ...
        angle = round(joystick1[0], 2)

        trig0 = 0

        if (triggers[0] > 0):
            trig0 = num_to_range(triggers[0], 0, 2, 0, 0.2)
        else:
            trig0 = 0
        ...
        if (triggers[1] > 0):
            trig1 = num_to_range(triggers[1], 0, 2, 0, 0.2)
        else:
            trig1 = 0
        ...
        t = num_to_range(throttle, -1, 1, 0, 180)
        s = num_to_range(angle, -1, 1, 0, 180)

        print(str(t)+" "+str(s))

        DrivingServo.angle = int(t)
        SteeringServo.angle = int(s)
